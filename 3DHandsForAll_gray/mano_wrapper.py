...
class mano_fitter:
    ...
    def set_shape(self, mano_shape):
        self.mano_model.set_input_shape(torch.from_numpy(mano_shape).cpu())
    ...
    def set_mano(self, mano_np, root_idx = None):
        xyz_tensor = torch.from_numpy(mano_np[:3]).cpu()
        input_rot_tensor = torch.from_numpy(mano_np[3:6]).cpu()
        input_pose_tensor = torch.from_numpy(mano_np[6:]).cpu()
    ...
    def fit_xyz_root_init(self, kpts_2d_glob, is_tracking):
        ...
        self.mano_model.set_xyz_root(torch.tensor([0, 0, 50.0]).view(1, -1).cpu())
