...
class Model(nn.Module):
    def __init__(self, mano_path, renderer, renderer2, device, batch_size, root_idx = 0):
        ...
        # gray
        self.K = torch.tensor([[390.66532698,   0.0,         208.63954441],
            [0.0,            394.95706975,204.82122184],
            [0.0,            0.0,         1.0]], dtype=torch.float32).cpu()
        ...
        self.xy_root = nn.Parameter(torch.tensor([0.0, 0.0], dtype=torch.float32).cpu().repeat(self.batch_size, 1))
        self.z_root = nn.Parameter(torch.tensor([50], dtype=torch.float32).cpu().repeat(self.batch_size, 1))
        
        self.is_rot_only = False
        self.input_rot = nn.Parameter(utils.clip_mano_hand_rot(torch.zeros(self.batch_size, 3).cpu()))
        self.input_pose = nn.Parameter(torch.zeros(self.batch_size, 45).cpu())
        self.input_shape = nn.Parameter(torch.zeros(self.batch_size, 10).cpu())
        self.shape_adjusted = utils.clip_mano_hand_shape(self.input_shape)
        self.camera_position = torch.tensor([0.0, 0.0, 0.0], dtype=torch.float32).cpu().repeat(self.batch_size, 1)
        self.ups = torch.tensor([0.0,-1.0,1.0], dtype=torch.float32).cpu().repeat(self.batch_size, 1)
        self.ats = torch.tensor([0.0,0.0,1.0], dtype=torch.float32).cpu().repeat(self.batch_size, 1)
        ...
        self.scale = torch.tensor([[self.compute_normalized_scale(hand_joints)]]).cpu()
        self.init_loss_mode()
    
    def set_kpts_3d_glob_leap(self, data):
        self.kpts_3d_glob_leap = torch.from_numpy(data).cpu()[1:, :].reshape((21, 3)) # ignore palm joint
    ...
    def set_kpts_3d_glob_leap_no_palm(self, data, with_xyz_root = False):
        self.kpts_3d_glob_leap = torch.from_numpy(data).cpu().reshape((21, 3))
    ...
    def set_image_ref(self, image_ref_new):
        self.image_ref, _ = torch.max(torch.tensor(image_ref_new[None,...] > 128, dtype=torch.float32).cpu(), -1)

    def set_kpts_2d_glob_gt_val(self, kpt_2d_idx, kpt_2d_val):
        self.kpts_2d_idx_set.add(kpt_2d_idx)
        self.kpts_2d_glob_ref[:, kpt_2d_idx] = torch.tensor(kpt_2d_val).cpu()
    ...
    def reset_parameters(self, keep_mano = False):
        if not keep_mano:
            self.xy_root.data = torch.tensor([0.0, 0.0], dtype=torch.float32).cpu().repeat(self.batch_size, 1)
            self.z_root.data = torch.tensor([50], dtype=torch.float32).cpu().repeat(self.batch_size, 1)
            self.input_rot.data = utils.clip_mano_hand_rot(torch.zeros(self.batch_size, 3).cpu())
            self.input_pose.data = torch.zeros(self.batch_size, 45).cpu()
            self.input_shape.data = torch.zeros(self.batch_size, 10).cpu()
            self.shape_adjusted = self.input_shape
        self.kpts_2d_idx_set = set()
        self.kpts_2d_glob_ref[...] = 0.0
