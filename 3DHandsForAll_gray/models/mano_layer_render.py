...
class Model(nn.Module):
    def __init__(self, mano_path, renderer, device, batch_size, root_idx = 0):
        ...
        self.input_pose = torch.zeros(self.batch_size, 48).cpu()
        self.input_shape = torch.zeros(self.batch_size, 10).cpu()
        self.camera_position = torch.tensor([0.0, 0.0, 0.0], dtype=torch.float32).cpu().repeat(1, 1)
        self.ups = torch.tensor([0.0,-1.0,1.0], dtype=torch.float32).cpu().repeat(1, 1)
        self.ats = torch.tensor([0.0,0.0,1.0], dtype=torch.float32).cpu().repeat(1, 1)
        ...
        self.texture = TexturesVertex(verts_features=torch.ones(1, 778, 3).cpu())
    ...
    def forward(self):
        ...
        #textures0 = TexturesVertex(verts_features=verts_rgb0.cpu())
        
        #verts_rgb1 = torch.ones_like(verts[1:2])
        #textures1 = TexturesVertex(verts_features=verts_rgb1.cpu())
