...
class mano_fitter:
    ...
    def fit_3d_can_init(self, kpts_3d_can, is_tracking):
        ...
        # if is_tracking and hasattr(self, "prev_pose") and hasattr(self, "prev_rot") and hasattr(self, "prev_shape"):
        #     with torch.no_grad():
        #         self.mano_model.input_pose.copy_(self.prev_pose)
        #         self.mano_model.input_rot.copy_(self.prev_rot)
        #         self.mano_model.input_shape.copy_(self.prev_shape)
        # else:
        #     with torch.no_grad():
        #         self.mano_model.input_pose.zero_()
        #         self.mano_model.input_rot.zero_()
        #         self.mano_model.input_shape.zero_()
        ...
        # # === New: 儲存這一幀的結果供下一幀追踪用 ===
        # self.prev_pose = self.mano_model.input_pose.detach().clone()
        # self.prev_rot = self.mano_model.input_rot.detach().clone()
        # self.prev_shape = self.mano_model.input_shape.detach().clone()
    ...
    def get_rendered_img(self):
        self.mano_model.change_render_setting(True)
        out1, out2, out3, out4, out5, img_rendered = self.mano_model()
        # print("Output 1:", out1)  # 可能是初始 loss 項或佔位符
        # print("Output 2:", out2)  # 可能是總 loss 值（用於訓練）
        # print("Output 3:", out3)  # 可能是MANO pose+shape+cam 參數向量
        # print("Output 4:", out4)  # 可能是子 loss，例如 MSE 或 regularization
        # print("Output 5:", out5)  # ??
        # print("Rendered Image (Output 6):", img_rendered) # 渲染圖像，包含手部 mesh 和關鍵點
