...
def clip_mano_hand_pose(pose_tensor):
    pose_min_tensor = torch.tensor(params.rot_min_list[3:]).cpu()
    pose_max_tensor = torch.tensor(params.rot_max_list[3:]).cpu()
...
def leapdata_scaled2ego(leap, ego):
    ...
    return torch.from_numpy(res).cpu()
    
def get_pose_constraint_tensor():
    pose_mean_tensor = torch.tensor(params.pose_mean_list).cpu()
    pose_reg_tensor = torch.tensor(params.pose_reg_list).cpu()
